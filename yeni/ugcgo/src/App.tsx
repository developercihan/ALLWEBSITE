...
import { Instagram, Linkedin, Twitter, Target, TrendingUp, LayoutTemplate, Menu, X } from 'lucide-react';
...
const Navbar = () => {
  ...
  const navItems = ["Ana Sayfa", "Hizmetlerimiz", "Yaklaşımımız", "Sonuçlar"];

  return (
    <motion.nav 
      style={{ backgroundColor, backdropFilter, paddingTop: py, paddingBottom: py }}
      className="fixed top-0 inset-x-0 z-[100] px-4 md:px-12 xl:px-28 flex flex-col justify-center border-b border-white/5 isolate"
    >
      <div className="flex items-center justify-between w-full relative z-50">
        <div className="flex items-center gap-3 md:gap-4 hover:opacity-80 transition-opacity cursor-pointer">
          ...
          <div className="flex items-baseline gap-1">
            <span className="font-bold text-lg md:text-xl tracking-tight">UGCgo</span>
        ...
        <div className="flex items-center gap-2 md:gap-3">
          <div className="hidden md:flex items-center gap-2 mr-2">
            {[Instagram, Linkedin, Twitter].map((Icon, idx) => (
              <a key={idx} href="#" className="w-8 h-8 rounded-full flex items-center justify-center bg-white/5 hover:bg-white/15 border border-white/10 transition-all group shadow-[inset_0_0_10px_rgba(255,255,255,0.02)]">
                <Icon className="w-4 h-4 text-white/70 group-hover:text-white transition-colors" strokeWidth={2} />
              </a>
            ))}
          </div>
          ...
          <button 
            ...
            className="lg:hidden w-10 h-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-white active:scale-95 transition-transform"
          >
          ...
          <motion.button 
            ...
            whileTap={{ scale: 0.98 }}
            className="hidden sm:flex bg-white text-black font-bold text-[11px] tracking-widest px-6 py-2.5 rounded-full shadow-lg"
          >
            İLETİŞİM
          </motion.button>
        </div>
      ...
      <AnimatePresence>
        {isMenuOpen && (
          <motion.div
            ...
            className="lg:hidden overflow-hidden bg-black/95 backdrop-blur-2xl absolute top-full left-0 right-0 border-b border-white/10 shadow-2xl"
          >
            <div className="flex flex-col p-8 gap-6 items-center text-center">
              ...
              <div className="flex gap-4 pt-4 border-t border-white/5 w-full justify-center">
                {[Instagram, Linkedin, Twitter].map((Icon, idx) => (
                  <a key={idx} href="#" className="w-12 h-12 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center">
...
const HeroSection = () => (
  <section className="relative min-h-[100dvh] w-full flex flex-col items-center justify-center overflow-hidden">
    ...
    <main className="relative z-10 flex flex-col items-center justify-center text-center px-4 pt-32 pb-32 max-w-4xl mx-auto h-full w-full">
      <motion.div {...fadeUp(0.1)} className="flex items-center gap-3 md:gap-4 mb-6 md:mb-8 bg-white/5 backdrop-blur-lg px-4 py-2.5 rounded-full border border-white/10 shadow-xl">
        <div className="flex -space-x-2 md:-space-x-3">
          {["avatar-1.png", "avatar-2.png", "avatar-3.png"].map((src, i) => (
            <div key={i} className={`w-6 h-6 md:w-9 md:h-9 rounded-full border-2 border-black overflow-hidden relative shadow-inner ${i === 0 ? 'bg-white/20 z-[3]' : i === 1 ? 'bg-white/40 z-[2]' : 'bg-white/60 z-[1]'}`}>
              <img src={`/avatar-${i+1}.png`} alt="Avatar" className="w-full h-full object-cover mix-blend-overlay" />
            </div>
          ))}
        </div>
        <span className="text-[10px] md:text-sm font-semibold text-white/90 tracking-wide text-left leading-tight">7.000+ Başarı Hikayesi</span>
      </motion.div>
      
      <motion.h1 {...fadeUp(0.2)} className="text-[14vw] sm:text-[10vw] md:text-[88px] leading-[0.95] md:leading-[0.9] font-medium tracking-tight md:tracking-[-3px] w-full max-w-4xl mb-6 md:mb-8">
        İzleyiciyi Yakala.<br /> Satışa <span className="font-serif italic font-normal text-white/60">Dönüştür</span>
      </motion.h1>
      
      <motion.p {...fadeUp(0.3)} className="text-sm sm:text-base md:text-xl text-white/50 max-w-xl mb-10 md:mb-14 leading-relaxed px-6 sm:px-0">
        Kaydırma alışkanlıklarını durduracak dijital mıknatıslar üretiyor ve bu ilgiyi doğrudan ciroya çeviriyoruz.
      </motion.p>
      
      <motion.form {...fadeUp(0.4)} className="bg-white/5 border border-white/10 backdrop-blur-xl flex flex-col sm:flex-row items-center p-1.5 rounded-[28px] sm:rounded-full w-full max-w-[92%] sm:max-w-lg mx-auto gap-1.5 sm:gap-0">
        <input 
          type="url" 
          placeholder="Web sitenizin adresi" 
          className="bg-transparent border-none outline-none flex-1 px-5 sm:px-6 py-4 sm:py-0 w-full text-sm text-foreground placeholder:text-white/30 text-center sm:text-left rounded-full"
          required
        />
        <motion.button 
          whileHover={{ scale: 1.02 }}
          whileTap={{ scale: 0.98 }}
          type="submit" 
          className="bg-white text-black font-bold text-[10px] md:text-xs tracking-[0.15em] px-8 py-4 md:py-4 rounded-full w-full sm:w-auto shrink-0 shadow-2xl"
        >
          ÜCRETSİZ ANALİZ
        </motion.button>
      </motion.form>
...
const ServicesSection = () => {
  ...
  return (
    <section className="pt-20 md:pt-40 lg:pt-64 pb-12 md:pb-16 px-4 max-w-7xl mx-auto flex flex-col items-center text-center">
      <motion.h2 {...fadeUp(0.1)} className="text-3xl sm:text-5xl md:text-7xl font-medium tracking-tight mb-6">
        Performans Odaklı <br className="sm:hidden" /><span className="font-serif italic font-normal text-white/50">Video Üretimi</span>
      </motion.h2>
      
      <motion.p {...fadeUp(0.2)} className="text-white/40 text-sm sm:text-lg max-w-2xl mx-auto mb-12 md:mb-24 px-4 sm:px-0 leading-relaxed">
        Tıklama oranından (CTR) satış maliyetine kadar her veriyi takip eden ve sanatı ölçülebilir sonuçlarla birleştiren bir dönüşüm merkezliyiz.
      </motion.p>
      
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 sm:gap-16 w-full mb-20 px-4 md:px-12">
        {cards.map((card, i) => {
...
const MissionSection = () => (
  <section className="pt-0 pb-32 md:pb-44 px-4 max-w-5xl mx-auto flex flex-col items-center text-center">
    <motion.div {...fadeUp(0.1)} className="w-[85vw] max-w-[420px] aspect-square rounded-full mb-20 md:mb-32 relative p-4 md:p-6 bg-white/[0.03] border border-white/10 shadow-[0_0_100px_rgba(255,255,255,0.1)] backdrop-blur-xl">
      <div className="absolute inset-0 rounded-full border-[2px] border-white/10 shadow-[inset_0_0_60px_rgba(255,255,255,0.05)] pointer-events-none"></div>
      <div className="w-full h-full rounded-full overflow-hidden relative shadow-[inset_0_0_40px_rgba(0,0,0,0.8)] border border-white/5 bg-black">
        <div className="absolute inset-0 rounded-full shadow-[inset_0_0_60px_rgba(255,255,255,0.2)] pointer-events-none z-20 mix-blend-overlay"></div>
    ...
    <div className="text-left w-full max-w-4xl mx-auto px-6">
      <ScrollText 
        ...
        className="text-xl sm:text-2xl md:text-5xl font-medium tracking-tight leading-snug md:leading-tight"
      />
      ...
      <ScrollText 
        ...
        className="text-lg sm:text-xl md:text-3xl font-medium mt-8 md:mt-12 text-white/40 leading-snug md:leading-snug"
      />
    </div>
  </section>
);
...
const SolutionSection = () => {
  ...
  return (
    <section className="py-24 md:py-44 border-t border-white/10 px-4 sm:px-6 max-w-7xl mx-auto">
      <div className="mb-8 md:mb-16 text-center md:text-left px-4">
        <motion.p {...fadeUp(0.1)} className="text-[10px] md:text-xs tracking-[3px] uppercase text-white/40 mb-4 md:mb-6 font-semibold">
          ÇÖZÜM
        </motion.p>
        <motion.h2 {...fadeUp(0.2)} className="text-2xl sm:text-4xl md:text-6xl font-medium tracking-tight max-w-2xl mx-auto md:mx-0">
          Satış odaklı <span className="font-serif italic font-normal text-white/50">Video-First</span> yaklaşım
        </motion.h2>
      </div>

      <motion.div {...fadeUp(0.3)} className="w-full aspect-video sm:aspect-[2/1] md:aspect-[3/1] rounded-[24px] md:rounded-[32px] overflow-hidden relative mb-12 md:mb-[72px] border border-white/10 shadow-2xl">
        <video 
          autoPlay 
          loop 
          muted 
      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex flex-col lg:flex-row items-center gap-16 lg:gap-24">
          <div className="w-full lg:w-1/2">
            <motion.p {...fadeUp(0.1)} className="text-[10px] md:text-xs tracking-[3px] uppercase text-white/40 mb-6 font-semibold">
              MÜŞTERİ YORUMLARI
            </motion.p>
            <motion.h2 {...fadeUp(0.2)} className="text-2xl sm:text-4xl md:text-6xl font-medium tracking-tight mb-8 md:mb-12">
              Sonuçlarımızı <br className="sm:hidden" /><span className="font-serif italic font-normal text-white/30">Ortaklarımız Anlatsın</span>
            </motion.h2>
            <div className="flex gap-2.5">
              {testimonials.map((_, i) => (
                <button 
                  key={i} 
                  onClick={() => setActive(i)}
                  className={`w-10 h-1 bg-white/10 rounded-full transition-all duration-500 overflow-hidden relative pt-2 pb-2 -mt-2 -mb-2`}
                >
                  <div className={`absolute inset-0 bg-white/10 transition-opacity ${active === i ? 'opacity-0' : 'opacity-100'}`} />
                  {active === i && (
                    <motion.div 
                      layoutId="activeTab"
                      className="absolute inset-x-0 top-[6px] h-1 bg-white" 
                    />
                  )}
                </button>
              ))}
          ...
          <div className="w-full lg:w-1/2 relative">
            <div className="relative isolate min-h-[340px] md:min-h-[400px] flex items-center">
              <motion.div 
                key={active}
                ...
                className="w-full bg-white/[0.02] backdrop-blur-2xl border border-white/[0.08] rounded-[40px] p-8 md:p-14 relative overflow-hidden shadow-3xl"
              >
                {/* Decoration */}
  ...
  return (
    <section className="relative py-24 md:py-44 border-t border-white/10 overflow-hidden flex items-center justify-center min-h-[60vh] md:min-h-[70vh]">
      ...
      <div className="relative z-10 flex flex-col items-center text-center px-4 max-w-3xl w-full">
        ...
        <motion.h2 {...fadeUp(0.2)} className="text-4xl sm:text-5xl md:text-7xl font-serif italic text-foreground mb-6 md:mb-10 font-normal">
          Büyümeye <br className="sm:hidden" /> Hazır Mısınız?
        </motion.h2>
        
        <motion.p {...fadeUp(0.3)} className="text-white/40 text-sm md:text-lg mb-10 md:mb-14 max-w-xl px-8 leading-relaxed">
          E-ticaret markanızı bir sonraki seviyeye taşımak için veri odaklı video stratejilerimizle tanışın.
        </motion.p>
        
        <motion.div {...fadeUp(0.4)} className="flex flex-col sm:flex-row items-center gap-3 md:gap-4 w-full sm:w-auto px-6 sm:px-0">
          <motion.button 
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
            className="bg-white text-black font-bold text-[11px] md:text-xs tracking-[0.2em] rounded-full px-10 py-5 sm:py-4 w-full sm:w-auto shadow-2xl"
          >
            STRATEJİ GÖRÜŞMESİ
          </motion.button>
          <motion.button 
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
            className="bg-white/5 text-white font-bold text-[11px] md:text-xs tracking-[0.2em] rounded-full px-10 py-5 sm:py-4 w-full sm:w-auto border border-white/10 backdrop-blur-md"
          >
            PORTFOLYO
          </motion.button>
        </motion.div>
